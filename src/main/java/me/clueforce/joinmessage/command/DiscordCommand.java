...
public class DiscordCommand implements CommandExecutor {
    ...
    public DiscordCommand(ConfigUtils configUtils, LangUtils langUtils) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!configUtils.getConfig().getBoolean("options.discord-enabled" +
                "")) {
            sender.sendMessage(langUtils.getMessage("command-disabled", true));
            return true;
        }

        for (String s : configUtils.getConfig().getStringList("discord")) {
